...
VALUES ($1, $2, $3, $4)
RETURNING id, company_id, name, email, password, created_at, updated_at
`
...
SELECT id, company_id, name, email, password, created_at, updated_at
FROM users
WHERE email = $1
`
...
SELECT count(*) OVER () AS total_items, sub_query.id, sub_query.company_id, sub_query.name, sub_query.email, sub_query.password, sub_query.created_at, sub_query.updated_at
FROM (SELECT id, company_id, name, email, password, created_at, updated_at
      FROM users
      ORDER BY CASE
